...
import { Search, Plus, Download, PanelLeft } from "lucide-react";
...
export function Header({
  ...
  return (
    <header className="h-14 bg-white/90 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-20 select-none">
      
      {/* Left: Sidebar Collapse Toggle + New Task Button */}
      <div className="flex items-center space-x-2">
        <button
          onClick={onToggleSidebar}
          className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-500 hover:text-slate-900 transition border border-slate-200"
        ...
        <button
          ...
          className="bg-[#3559E0] hover:bg-[#2c4ac0] text-white font-bold px-3 py-1.5 rounded-xl text-xs flex items-center space-x-1.5 transition shadow-xs shadow-[#3559E0]/20"
        >
      ...
      {/* CENTER: PURE TITLE 'Ergo' ONLY */}
      <div className="absolute left-1/2 -translate-x-1/2 flex items-center">
        <span className="font-extrabold text-lg text-slate-900 tracking-tight">Ergo</span>
      </div>
